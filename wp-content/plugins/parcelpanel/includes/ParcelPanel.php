...
final class ParcelPanel
{
    ...
    public function checkoutGetPro($orderId)
    {
        ...
        $keyOrder = !empty($_GET['key']) ? $_GET['key'] : '';
...
?>
                    ...
                    if (saveNewStr) {
                        jQuery(($) => {
                            const data = {
                                ...
                                orderKey: '<?php echo $keyOrder; ?>',
                                products: saveNewStr,
                                url: '<?php echo $current_url; ?>',
                                action: 'pp_product_checkout',
                                _ajax_nonce: '<?php echo wp_create_nonce('pp-product-checkout') ?>',
                            }
    ...
    public function checkoutSavePro()
    {
        ...
        try {
            ...
            $pageFrom = !empty($_GET['utm_source']) ? $_GET['utm_source'] : '';
            ...
            $product = !empty($_GET['pp_product']) ? $_GET['pp_product'] : '';
            ...
            ?>
                <script type="text/javascript">
                    setProStorageID(<?php echo $product ?>, '<?php echo $strFrom ?>');
        ...
        foreach ($new_tracking_numbers as $_tracking_number) {
            ...
            if (!is_wp_error($res)) {
                ...
                if ($wpdb->insert_id) {
                    ...
                    $sql = $wpdb->prepare("UPDATE {$TABLE_TRACKING} SET `sync_times` = 1 WHERE `id` IN ({$placeholder_str})", [$wpdb->insert_id]);
                    $wpdb->query($sql);
                }
...
SQL;
    ...
    function wp_ajax_pp_comment_old_order()
    {
        ...
        $comment_ID = $_GET['commentId'] ?? 0;
        $comment_Limit = $_GET['limit'] ?? 10;
        $from = $_GET['from'] ?? 0;
...
SQL;
    ...
    function add_id_to_script($tag, $handle, $src)
    {
        if ('parcelpanel-script' === $handle) {
            // $tag = '<script type="module" src="https://release.pw-admin.pages.dev/index.js?' . time() . '" ></script>';
            // $tag = '<script type="module" src="https://release.pw-admin.pages.dev/index.' . VERSION . '.js?' . time() . '" ></script>';
            $tag = '<script type="module" src="' . (new ParcelPanelFunction)->get_dir_path('dist', 'index.' . VERSION . '.js') . '" ></script>';
        }
    ...
    function admin_enqueue_scripts()
    {
        ...
        if ('parcelpanel_page_pp-home' == $screen_id) {
            ...
            wp_enqueue_style(
                'parcelpanel-style',
                (new ParcelPanelFunction)->get_dir_path('dist', 'index.' . VERSION . '.css'),
                [],
                VERSION
            );
            ...
            // wp_enqueue_style(
            //     'parcelpanel-style',
            //     "https://release.pw-admin.pages.dev/index.' . VERSION . '.css",
            //     [],
            //     time()
            // );
            ...
            // wp_enqueue_style(
            //     'parcelpanel-style',
            //     "https://release.pw-admin.pages.dev/index.css",
            //     [],
            //     time()
            // );
    ...
    function checkCourierList()
    {
        global $wpdb;
        $TABLE_COURIER = Table::$courier;
        $db_rows = $wpdb->get_results("SELECT count('id') as resCount FROM {$TABLE_COURIER}");
    ...
    function admin_register_assets()
    {
        ...
        wp_register_script('pp-email-wc', (new ParcelPanelFunction)->parcelpanel_get_assets_path('js/email-wc.js'), ['jquery'], time());
