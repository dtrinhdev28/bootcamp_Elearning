...
SQL;
        ...
        if ($this->original_tracking_id) {
            ...
            $sql = $wpdb->prepare("UPDATE {$TABLE_TRACKING} SET `sync_times` = 1 WHERE `id` IN ({$placeholder_str})", [$this->original_tracking_id]);
            $res = $wpdb->query($sql);
        }
        ...
        while ($run_times--) {
            ...
            if (!$is_no_quota) {
                ...
                if ($post_ids) {
                    ...
                    $shipment_order_ids = $wpdb->get_col($wpdb->prepare($SQL, $post_ids));
