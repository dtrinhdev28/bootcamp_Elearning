...
class Common
{
    ...
    public function shipmentChange($order_message = [], $type = 0)
    {
        ...
        if ($save_in_order_meta) {
            ...
            $shipping_numbers_str = json_encode($shipping_numbers_arr);

            // (new ParcelPanelFunction)->parcelpanel_log(json_encode([
            //     'test' => 'add shipment 222',
            //     'type' => $type,
            //     'tracking_number' => $tracking_number,
            //     'shipping_numbers_arr' => $shipping_numbers_arr,
            // ], 320));
    ...
    private function save_wp_comment($order_id, $fulfilled_at, $comment_content)
    {
        $args = array(
            ...
            // "comment_author_url" => "",
            // "comment_author_IP" => "",
            "comment_date" => date('Y-m-d H:i:s', $fulfilled_at),
            "comment_date_gmt" => date('Y-m-d H:i:s', $fulfilled_at),
