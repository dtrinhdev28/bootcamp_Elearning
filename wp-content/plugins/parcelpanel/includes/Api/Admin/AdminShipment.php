...
class AdminShipment
{
    ...
    private function deleteOrderShipment($order_id, $tracking_number)
    {
        ...
        if (!empty($tracking_number)) {
            ...
            $del_tracking_id = [];
            $tracking_item = $wpdb->get_results($wpdb->prepare("SELECT tracking_id FROM {$TABLE_TRACKING_ITEMS} WHERE order_id=%d", $order_id));
            foreach ($tracking_item as $v) {
                $tracking_id = $v->tracking_id ?? 0;
                if (!empty($tracking_id)) {

                    $tracking_item_arr = $wpdb->get_row($wpdb->prepare("SELECT tracking_number FROM {$TABLE_TRACKING} WHERE id=%d", $tracking_id));
