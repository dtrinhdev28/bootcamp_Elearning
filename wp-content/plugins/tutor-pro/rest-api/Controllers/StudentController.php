...
class StudentController extends BaseController {
	...
	public function get( WP_REST_Request $request ) {
		...
		} elseif ( 'quizzes' === $sub_resource ) {
			return $this->get_quizzes( $params );
	...
	/**
	 * Get student quizzes API request
	 *
	 * @since 2.6.2
	 *
	 * @param array $params request array.
	 ...
	 * @return WP_REST_Response|WP_Error
	 */
	public function get_quizzes( $params ) {
		// Extract fillable fields.
		$params = array_intersect_key( $params, array_flip( $this->fillable_fields ) );
		...
		try {
			$quizzes = tutor_utils()->get_all_quiz_attempts_by_user( $user_id );

			return $this->response(
				$this->code_read,
				__( 'Quizzes retrieved successfully', 'tutor-pro' ),
				$quizzes
			);
		} catch ( \Throwable $th ) {
			return $this->response(
				$this->code_read,
				__( 'Quizzes retrieved failed', 'tutor-pro' ),
				$th->getMessage(),
				$this->server_error_code
			);
		}
